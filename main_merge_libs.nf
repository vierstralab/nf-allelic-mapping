...
include  { waspRealigning; add_snp_files_to_meta } from './main'
...
process merge_by_indiv_and_cell_type {
        ...
        output:
        	tuple val(indiv_id), val(ag_placeholder), file(name), file("${name}.crai")

        script:
	ag_placeholder="${indiv_id}_${cell_type}"
	name = "${ag_placeholder}.cram"
        """
        samtools merge -f -@${task.cpus} -O CRAM --write-index \
